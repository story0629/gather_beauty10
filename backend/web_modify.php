...
<body>
	<?php if (isset($_SESSION["USER"])){ ?>
		<?php if ($_SESSION["LEVEL"]==0) { ?>
			...
			<div class="container backend web_modify">
				<h1>修正平面設計作品</h1>
					<div class="form-group">
						<label class="col-md-2 col-sm-12 col-xs-12">縮圖</label>
						<?php if($row["small_img"]=="GG"){ ?>
							<form action="upload-web.php" method="post"  enctype="multipart/form-data">
						...
						<?php }else{ ?>
								<img src="../images/web/<?php echo $row["small_img"]; ?>" class="thumb">
								<a href="delete-web.php?id=<?php echo $row["id"]; ?>&images=<?php echo $row["small_img"]; ?>&i=1" class="fa fa-times fa-lg"></a>
						<?php } ?>
					</div>
					<div class="form-group">
						...
						<?php }else{ ?>
								<img src="../images/web/<?php echo $row["images1"]; ?>" class="thumb">
								<a href="delete-web.php?id=<?php echo $row["id"]; ?>&images=<?php echo $row["images1"]; ?>&i=2" class="fa fa-times fa-lg"></a>
						<?php } ?>
					</div>
					<div class="form-group">
						...
						<?php }else{ ?>
								<img src="../images/web/<?php echo $row["images2"]; ?>" class="thumb">
								<a href="delete-web.php?id=<?php echo $row["id"]; ?>&images=<?php echo $row["images2"]; ?>&i=3" class="fa fa-times fa-lg"></a>
						<?php } ?>
					</div>
				<form action="web_modify_OK.php" method="post" class="form-horizontal" enctype="multipart/form-data">
					<input type="hidden" name="id" value="<?php echo $row["id"]; ?>">
					<div class="form-group">
		...
		<?php }else{ header("Location:web_check.php"); } ?>
	<?php
		}else{
